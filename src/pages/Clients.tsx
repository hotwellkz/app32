...
export const Clients: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToClients(
      (allClients) => {
        // При получении клиентов, проверяем и обновляем видимость иконок
        allClients.forEach(async (client) => {
          if (client.status === 'built' && client.isIconsVisible !== false) {
            await updateClientVisibility(client.id, false);
          }
        });
  ...
  const updateClientVisibility = async (clientId: string, isVisible: boolean) => {
    try {
      ...
      if (clientDoc.exists()) {
        ...
        batch.update(clientRef, { isIconsVisible: isVisible });

        const [projectsQuery, clientsQuery] = [
          query(
            collection(db, 'categories'),
            where('title', '==', `${clientData.lastName} ${clientData.firstName}`),
            where('row', '==', 3)
          ),
          query(
            collection(db, 'categories'),
      ...
      showErrorNotification('Ошибка при обновлении видимости');
    }
  };
  ...
  const handleStatusChange = async (clientId: string, newStatus: 'building' | 'deposit' | 'built') => {
    try {
      ...
      const isVisible = newStatus !== 'built';
      await updateClientVisibility(clientId, isVisible);
